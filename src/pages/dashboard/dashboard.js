...
function Dashboard() {
	...
	// server chart
	var serverChartData = [
		{ name: 'Memoria Ram', data: [randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(),randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo(), randomNo()]},
	...
	var themeFont = (getComputedStyle(document.body).getPropertyValue('--bs-body-font-family')).trim();
  	...
  	return {
    	...
    	dataLabels: { enabled: false },
			grid: { show: true, borderColor: 'white' },
			stroke: { show: false },
    	colors: ['rgba('+ inverseRgb +', .65)', themeColor],
			legend: { fontFamily: themeFont, labels: { colors:'white' } },
			xaxis: {
			...
			<div className="row">
				...
				<div className="col-xl-6">
					<Card className="mb-3">
						<CardBody>
							<div className="d-flex fw-bold small mb-3">
								<span className="flex-grow-1">ORIGENES DE CONEXIONES</span>
								<CardExpandToggler />
							</div>
							<div className="ratio ratio-21x9 mb-3">
									...
									<table className="w-100 small mb-0 text-truncate text-inverse text-opacity-60">
										<thead>
											<tr className="text-inverse text-opacity-75">
												<th className="w-50">PAISES</th>
												<th className="w-25 text-end">VISITAS</th>
				...
				<div className="col-xl-6">
					<Card className="mb-3">
						<CardBody>
							<div className="d-flex fw-bold small mb-3">
								<span className="flex-grow-1">ULTIMOS ESTUDIOS REALIZADOS</span>
								<CardExpandToggler />
							</div>
				...
				<div className="col-xl-6">
					<Card className="mb-3">
						<CardBody>
							<div className="d-flex fw-bold small mb-3">
								<span className="flex-grow-1">REGISTRO DE ACTIVIDADES</span>
								<CardExpandToggler />
							</div>
							<div className="table-responsive">
									...
									<tbody>
										{activityLogData && activityLogData.length > 0 ? (activityLogData.map((log, index) => (
											<tr key={index}>
												...
												<td><span className={'badge d-block rounded-0 pt-5px w-70px ' + ((log.highlight) ? 'bg-theme text-theme-900' : 'bg-inverse bg-opacity-25')} style={{ minHeight: '18px'}}>WEB</span></td>
												<td><a href="#/" className="text-decoration-none text-inverse"><i className="bi bi-search"></i></a></td>
											</tr>
										))) : (
											<tr>
